...
// ─────────────────────────────────────────────────────────────────────────────
// POST /generate — Generate, upload, persist, and email a PDF invoice
// Every async step is inside the try block. next(err) propagates to the
// centralized error handler. Cloudinary cleanup runs in the catch block.
// ─────────────────────────────────────────────────────────────────────────────
router.post('/generate', protect, async (req, res, next) => {
  ...
  try {
    ...
    if (!caseId || !clientId || !expenseIds || expenseIds.length === 0) {
      return res.status(400).json({ success: false, message: 'Missing required fields or expenses.' });
    }
    ...
    if (!caseDoc || !client) {
      return res.status(400).json({ success: false, message: 'Case or Client not found.' });
    }

    if (expenses.length !== expenseIds.length) {
      return res.status(400).json({ success: false, message: 'One or more expense IDs are invalid.' });
    }
    ...
    if (existingInvoice) {
      return res.status(400).json({
        success: false,
        message: 'One or more selected expenses are already included in a previous invoice.',
      });
    }
    ...
    const dueDateStr = new Date(Date.now() + 14 * 24 * 60 * 60 * 1000).toLocaleDateString('en-IN', {
      day: '2-digit', month: 'short', year: 'numeric',
    });
    ...
    // F. Save the Invoice document to MongoDB
    ...
    // Update Cloudinary asset with resolved invoiceNumber as the public_id
    ...
    try {
      ...
      console.warn('[WARN] Cloudinary rename failed, keeping temp URL:', renameErr.message);
    }

    savedInvoice.pdfUrl = pdfUrl;
    await savedInvoice.save();

    // G. Send the invoice email
    const mailOptions = {
    ...
    publicIdToCleanup = null; // All steps succeeded; no cleanup needed

    // H. Respond with the saved invoice
    return res.status(201).json(savedInvoice);

  } catch (error) {
    // Attempt Cloudinary cleanup if an asset was uploaded before the failure
    if (publicIdToCleanup) {
      cloudinary.uploader
        .destroy(publicIdToCleanup, { resource_type: 'raw' })
        .catch((e) => console.error('[ERROR] Cloudinary cleanup failed:', e.message));
    }
    return next(error);
  }
});
...
// ─────────────────────────────────────────────────────────────────────────────
// GET /case/:caseId — List all invoices for a specific case
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// GET /:id/download — Generate a signed Cloudinary URL for invoice PDF download
// ─────────────────────────────────────────────────────────────────────────────
router.get('/:id/download', protect, async (req, res, next) => {
  try {
    ...
    if (!invoice.pdfUrl) {
      return res.status(400).json({ success: false, message: 'PDF has not been generated for this invoice.' });
    }
...
// ─────────────────────────────────────────────────────────────────────────────
// PATCH /:id/status — Update invoice payment status (admin only)
// ─────────────────────────────────────────────────────────────────────────────
